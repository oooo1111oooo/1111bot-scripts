...
ERROR_LOG: list = []  # [(hhmmss, label, msg)]，最多 50 筆
MAX_ERROR_LOG = 50

def log_err(label, msg):
    """錯誤統一記錄：print 到 journalctl 同時存入 ERROR_LOG。"""
    ts = hhmmss()
    print(f"{label} {msg}")
    ERROR_LOG.append((ts, str(label), str(msg)[:100]))
...
SAVE_FIELDS = ("sym","dir","tf","lev","margin","pre","post",
               ...
               "algo_id","tp_px","sl_px","frame_base","move_n","move_n_bar","move_hist","last_move",
               "in_lag","in_slip","in_ref","in_poll_n","in_poll_s",
               "entry_bar_ts",
               "pending_px","pending_t",
               "last_force_mv_t")
...
async def get_best(iid, d):
    """取最佳買一(bid)/賣一(ask)：做多用 ask-1tick，做空用 bid+1tick。"""
    r = await pub(f"/api/v5/market/ticker?instId={iid}")
    data = r["data"][0]
    if d == "L":
        return Decimal(data["askPx"])   # 做多：埋伏在 ask 下方
    else:
        return Decimal(data["bidPx"])   # 做空：埋伏在 bid 上方
...
async def live_rows(sym, tf, need):
    ...
    for i, x in enumerate(ha):
        ...
        rows.append({"ts": int(x["ts"]), "dt": dt, "color": x["color"],
                     "body_pct": body, "o": float(kl[i]["o"]), "c": float(kl[i]["c"]),
...
def bar_line(r, want=None, v1=None, v2=None, sec=True):
    ...
    if want is None:
        return f"{act}{hm_} {n1}｜{n2}{lv}"
    wl = GRN if want == "G" else RED
    bad = "\u274c" if r.get("color") != want else ""
    return f"{wl}{hm_}{act} {n1}｜{n2}{bad}{lv}"
...
def bar_line_confirm(r, want=None, dp=None):
    """confirm 專用：參數燈號 時間 均K燈號 [❌] 開盤價 收盤燈號 漲跌%（*移行尾）"""
    act = GRN if r.get("color") == "G" else RED
    dt = str(r.get("dt") or "")
    hm_ = dt[11:16]
    lv = "*" if r.get("live") else ""
    rc = r.get("c"); ro = r.get("o")
    try:
        rlt = GRN if (rc is not None and ro is not None and float(rc) >= float(ro)) else RED
    except Exception:
        rlt = ""
    if ro is not None and dp is not None:
        ros = f"{float(ro):.{dp}f}"
    else:
        ros = pct(ro) if ro is not None else "-"
    try:
        chg = (float(rc) - float(ro)) / float(ro) * 100 if rc is not None and ro is not None else None
        chg_str = f"{chg:+.2f}%" if chg is not None else ""
    except Exception:
        chg_str = ""
    if want is None:
        return f"{act}{hm_}{act}    {ros}{rlt}{chg_str}{lv}"
    wl = GRN if want == "G" else RED
    pad = "🚫" if r.get("color") != want else "💚"
    return f"{wl}{hm_}{act}{pad}{ros}{rlt}{chg_str}{lv}"

def entry_lines_confirm(info, dr, dp=None):
    """confirm 畫面專用：無ATR，顯示原K開盤價+原K收盤燈號，*移行尾。"""
    opp = "R" if dr == "L" else "G"
    want = "G" if dr == "L" else "R"
    L = [bar_line_confirm(x, opp, dp) for x in info["pre_seg"]]
    try:
        pre_sum = sum((float(x["c"]) - float(x["o"])) / float(x["o"]) * 100
                      for x in info["pre_seg"] if x.get("c") and x.get("o"))
        L.append(f"　　　pre合計 {pre_sum:+.2f}%")
    except Exception:
        pass
    L.append("\u2504" * 18)
    for x in info["post_seg"]:
        L.append(bar_line_confirm(x, want, dp))
    try:
        post_sum = sum((float(x["c"]) - float(x["o"])) / float(x["o"]) * 100
                       for x in info["post_seg"] if x.get("c") and x.get("o"))
        L.append(f"　　　post合計 {post_sum:+.2f}%")
    except Exception:
        pass
    return L
...
def frame_shift(S, px, d):
    """依現價 tick-by-tick 追蹤：有利方向移動幾U，止盈止損跟著移幾U。
    沒有門檻，只要現價超過上次基準就立刻移動。"""
    try:
    ...
    if d == "L":
        delta = cur - base
        if delta <= 0: return None          # 沒漲，不動
        nsl = align(sl + delta, tick, "S")
    else:
        delta = base - cur
        if delta <= 0: return None          # 沒跌，不動
        nsl = align(sl - delta, tick, "L")
    if nsl == sl: return None               # tick 對齊後沒有變化
...
async def h_open(app, S, spec, iid, d, pos, info, k):
    ...
    if size < spec["minsz"]:
        ...
        S["alive"] = False
        return False
    # 初始限價：best ask - 1tick（做多）/ best bid + 1tick（做空）
    _best = await get_best(iid, d)
    tick = spec["tick"]
    limit_px = align(_best - tick, tick, "L") if d == "L" else align(_best + tick, tick, "S")
    t_send = time.time()                     # 送單時刻（延遲量測基準）
    r = await api("POST", "/api/v5/trade/order",
                  {"instId": iid, "tdMode": "isolated", "side": "buy" if d == "L" else "sell",
                   "posSide": pos, "ordType": "limit", "sz": str(size),
                   "px": str(limit_px),
    ...
    oid = r["data"][0]["ordId"]
    cur_px = limit_px
    S["pending_px"] = str(cur_px); S["pending_t"] = t_send
    save_state()
    # 截止時間 = signal bar 開盤 + 2×TF秒（下一根收盤）
    tf_dur = tf_sec(S["tf"])
    bar_deadline = (ref_ts / 1000 + 2 * tf_dur) if ref_ts is not None else (time.time() + tf_dur)
    # 每 LIMIT_POLL 秒：查成交 → 若未成交且現價不同 → 取消舊單 → 掛新單
    fpx = None
    while time.time() < bar_deadline:
        await asyncio.sleep(int(S["interval"]))
        # ── 查目前掛單狀態 ──
        if oid:
            ...
            if st.get("code") == "0" and st.get("data"):
                ...
                if dd.get("state") == "canceled":
                    oid = None   # 交易所取消 → 下方會重掛
        # ── 取 best ask/bid，偏移 1 tick 確保 maker ──
        new_best = await get_best(iid, d)
        new_px = align(new_best - tick, tick, "L") if d == "L" else align(new_best + tick, tick, "S")
        if new_px != cur_px:
            if oid:
                await api("POST", "/api/v5/trade/cancel-order", {"instId": iid, "ordId": oid})
                oid = None
            r2 = await api("POST", "/api/v5/trade/order",
            ...
            if r2.get("code") == "0":
                oid = r2["data"][0]["ordId"]
                cur_px = new_px
                S["pending_px"] = str(cur_px); save_state()
        elif not oid:
            # 現價未變但沒有掛單（前次被取消）→ 原價重掛
            r3 = await api("POST", "/api/v5/trade/order",
                           {"instId": iid, "tdMode": "isolated",
                            "side": "buy" if d == "L" else "sell",
                            "posSide": pos, "ordType": "limit", "sz": str(size),
                            "px": str(cur_px),
            ...
            if r3.get("code") == "0":
                oid = r3["data"][0]["ordId"]
    if fpx is None:
        # 本TF收盤前都沒成交 → 取消掛單，通知此輪進場失敗
        if oid:
            await api("POST", "/api/v5/trade/cancel-order", {"instId": iid, "ordId": oid})
        S.pop("pending_px", None); S.pop("pending_t", None); save_state()
    ...
    # ↓↓ 以下都是「已經成交」之後的收尾，任何錯誤都不得再往外拋，
    #    否則會像 t_send 那次一樣把策略打斷、留下沒人管的倉位。

    S["state"] = "持倉中"; S["pos_open"] = True
    S["pos_px"] = str(fpx); S["pos_ee"] = ee; S["pos_sz"] = str(size)
    S["pnl_hist"] = []                       # 逐根損益紀錄，供 /status 與出場核對
    S["frame_base"] = str(fpx); S["move_n"] = 0; S["move_n_bar"] = 0; S["move_hist"] = []; S["last_move"] = ee
    save_state()
    ...
    try:
        return await _h_open_report(app, S, spec, iid, d, pos, info, size, fpx, ee,
                                    ref_px, ref_ts, t_send)
    except Exception as e:
        print("h_open report fail", type(e).__name__, e)
        await notify(app, S["chat"],
            f"{E.BOT} {S['sym']} {E.dir_word(d)} 已進場 {fpx}｜{size}張\n"
            f"（明細訊息產生失敗：{type(e).__name__}，不影響持倉與止盈止損）")
        return True

async def _h_open_report(app, S, spec, iid, d, pos, info, size, fpx, ee,
                         ref_px, ref_ts, t_send):
    """進場後的通知與統計，與下單流程分離，確保不會影響交易。"""
    head = [f"{E.BOT} OKX均K｜{ACCT}", "事件：🔔 訊號進場",
            f"{E.dir_emoji(d)} {S['sym']} {d} {S['tf']}",
            strat_params(S["sym"], d) or ""]
    tick_dp = max(0, -S["spec"]["tick"].as_tuple().exponent)
    lines = entry_lines_confirm(info, d, tick_dp) if info else []
    lag = slip = None
    ...
    # 進場那根的燈號行
    ein_hm = datetime.fromtimestamp(ee, TZ8).strftime("%H:%M")
    d_color = GRN if d == "L" else RED
    fpx_str = f"{float(fpx):.{tick_dp}f}"
    tail = ["\u2504" * 18,
            f"{d_color}{ein_hm}{d_color} {fpx_str}{d_color} 📌",
    ...
    try:
        ...
        while S["alive"]:
            try:
                ...
                # 單輪出錯不終止策略：有持倉時止盈止損仍在交易所端，
                # 策略必須留著才能繼續移動框架與偵測出場。
                print("hloop round error", S.get("sym"), S.get("dir"),
                      type(e).__name__, e)
                await notify(app, chat,
                    f"{E.BOT} {E.LOSS} {S['sym']} {E.dir_word(d)} 本輪錯誤：{type(e).__name__}: {e}\n"
                    f"策略繼續運行，"
                    + ("止盈止損仍在交易所端" if S.get("pos_open") else "等下一根重試"))
                await asyncio.sleep(2)
    except asyncio.CancelledError:
        raise
    except Exception as e:
...
async def close_bookkeeping(app, S, reason):
    ...
    sym = S["sym"]; tf = S["tf"]; dr = d
    ein_hm = datetime.fromtimestamp(ee, TZ8).strftime("%H:%M")
    tick_dp = max(0, -spec["tick"].as_tuple().exponent)
    rows_all = db_latest(sym, tf, pre + post + 40)
    # 找進場根：優先用 entry_bar_ts 精確比對，fallback 才用時間範圍
    entry_idx = None
    entry_bar_ts = S.get("entry_bar_ts")
    if entry_bar_ts is not None:
        for i, row in enumerate(rows_all):
            if int(row.get("ts") or 0) == int(entry_bar_ts):
                entry_idx = i; break
    if entry_idx is None:
        ee_ts_ms = int(ee * 1000)
        tf_s = HA_TF.get(tf, (300,))[0]
        for i, row in enumerate(rows_all):
            rts = int(row.get("ts") or 0)
            if rts <= ee_ts_ms < rts + tf_s * 1000:
                entry_idx = i; break
    if entry_idx is None:
        for i, row in enumerate(rows_all):
            dt_hm = str(row.get("dt") or "")[11:16]
            if dt_hm == ein_hm:
                entry_idx = i; break
    # 計算 pre/post 合計
    pre_sum = post_sum = None
    if entry_idx is not None:
        try:
            _pre_rows = rows_all[max(0, entry_idx - pre - post):entry_idx - post]
            _post_rows = rows_all[entry_idx - post:entry_idx]
            pre_sum = sum((float(r["c"]) - float(r["o"])) / float(r["o"]) * 100
                         for r in _pre_rows if r.get("c") and r.get("o"))
            post_sum = sum((float(r["c"]) - float(r["o"])) / float(r["o"]) * 100
                          for r in _post_rows if r.get("c") and r.get("o"))
        except Exception:
            pass
    if entry_idx is not None:
        show_rows = rows_all[max(0, entry_idx - pre - post):]
    else:
        show_rows = rows_all[-(pre + post):]  if rows_all else []
    for i, row in enumerate(show_rows):
        abs_i = rows_all.index(row) if row in rows_all else i
        is_entry = (entry_idx is not None and abs_i == entry_idx)
        is_post_start = (entry_idx is not None and abs_i == entry_idx - post)
        if is_post_start:
            if pre_sum is not None:
                lines.append(f"　　　pre合計 {pre_sum:+.2f}%")
            lines.append("\u2504" * 18)
        if is_entry:
            if post_sum is not None:
                lines.append(f"　　　post合計 {post_sum:+.2f}%")
            lines.append("\u2504" * 18)
        act = GRN if row.get("color") == "G" else RED
        dt_hm = str(row.get("dt") or "")[11:16]
        ro = row.get("o"); rc = row.get("c")
        try:
            rlt = GRN if (rc is not None and ro is not None and float(rc) >= float(ro)) else RED
        except Exception:
            rlt = ""
        ros = f"{float(ro):.{tick_dp}f}" if ro is not None else "-"
        # 原K漲跌幅
        try:
            rchg = f"{(float(rc) - float(ro)) / float(ro) * 100:+.4f}%" if (ro and rc is not None) else ""
        except Exception:
            rchg = ""
        # 進場根前用 want 色，之後用 act 色
        if entry_idx is not None and abs_i < entry_idx:
            # 前段+後段（進場前）
            if abs_i < entry_idx - post:
                wl = GRN if dr == "L" else RED  # 反向色 for pre_seg... actually use opp
                opp = "R" if dr == "L" else "G"
                wl = GRN if opp == "G" else RED
            else:
                want = "G" if dr == "L" else "R"
                wl = GRN if want == "G" else RED
            bad = "\u274c" if abs_i >= entry_idx - post and row.get("color") != ("G" if dr == "L" else "R") else ""
        else:
            wl = act
            bad = ""
        pin = " 📌" if is_entry else ""
        lv = "*" if row.get("live") else ""
        lines.append(f"{wl}{dt_hm}{act} {ros}{rlt}{rchg}{bad}{lv}{pin}")
        if is_entry:
            lines.append(f"止盈{S.get('tp_px')}({pct(S['tp_pct'])}%) | 止損{S.get('sl_px')}({pct(S['sl_pct'])}%)")
            in_ref = S.get("in_ref")
            in_lag = S.get("in_lag")
            if in_ref:
                ref_line = f"掛單(收盤){in_ref}"
                if in_lag is not None:
                    try: ref_line += f" | 延遲{float(in_lag):.2f}s"
                    except Exception: pass
                lines.append(ref_line)
    # 真正出場行（實際成交時間 + 出場價 + 淨損益%）
    d_color = GRN if d == "L" else RED
    try:
        exit_hm = datetime.fromtimestamp(int(ph.get("uTime", 0)) / 1000, TZ8).strftime("%H:%M") if ph else hhmmss()[:5]
    except Exception:
        exit_hm = hhmmss()[:5]
    xpx_str = f"{float(xpx):.{tick_dp}f}"
    lines.append(f"{d_color}{exit_hm}{d_color} {xpx_str}{d_color}{npv:+.3f}% {ico}")
    # 框架移動歷史
        ...
        try:
            ...
            for S in due:
                ...
                if p is None:
                    ...
                    try:
                        ...
                        log_err(f"close_bookkeeping fail {S['sym']}", f"{type(e).__name__} {e}")
                    continue
                px = p.get("last") or p.get("markPx")
                if not px or not S.get("algo_id"): continue

                # --- 定時推格（每 FORCE_MV_INTERVAL 秒固定調整一次止損）---
                force_mv = None
                now_t = time.time()
                ...
                if now_t - last_force >= FORCE_MV_INTERVAL:
                    ...
                    try:
                        ...
                        force_shift = float(move_min) * 100
                        force_mv = (Decimal(str(S["tp_px"])), sl_f)
                    except Exception as e:
                        log_err(f"force_shift calc fail {S['sym']}", e)

                # --- 現價 tick-by-tick 追蹤（收盤秒跳過，兩套互不干擾）---
                mv = None if force_mv else frame_shift(S, px, S["dir"])
                ...
                S["_pending"] = (str(ntp), str(nsl), str(nbase), gain, bool(force_mv), str(px))
            ...
            if amends:
                ...
                for a, b, c, d2, S in amends:
                    if okn:
                        ntp, nsl, nbase, gain, is_bar, _mpx = S.pop("_pending")
                        S["tp_px"] = ntp; S["sl_px"] = nsl
                        S["frame_base"] = nbase
                        S["move_n"] = int(S.get("move_n", 0)) + 1
                        mtype = "定時" if is_bar else "現價"
                        mh = S.get("move_hist")
                        if not isinstance(mh, list): mh = []; S["move_hist"] = mh
                        mh.append({"t": hhmmss(), "type": mtype,
                                   "px": str(nbase), "sl": str(nsl)})
                        if is_bar:
                            S["move_n_bar"] = int(S.get("move_n_bar", 0)) + 1
...
def strat_params(sym, dr):
    """回傳參數字串，省略 /run 商品 方向，從槓桿倍數開始。"""
    S = STRATS.get(skey(sym, dr))
    if not S or not S.get("alive"):
        return None
    return (f"{S['lev']}x {pct(S['margin'])} "
...
async def strat_detail(S, sym, dr, mark_px=None):
    ...
    if S.get("pending_px") and not S.get("pos_open"):
        waited = int(time.time() - float(S.get("pending_t") or time.time()))
        L.append(f"狀態：⏳ 掛單中 {S['pending_px']}，已等 {waited}s")
        return L
    if not S.get("pos_open"):
        ...
        _dp = max(0, -S["spec"]["tick"].as_tuple().exponent)
        L += entry_lines_confirm(r, dr, _dp)
    ...
    ein_hm = datetime.fromtimestamp(ee, TZ8).strftime("%H:%M")
    pre = int(S["pre"]); post = int(S["post"])
    # 抓進場前 pre+post 根（已收線）+ 進場後所有根（含即時）
    rows_live = await live_rows(sym, tf, pre + post + 20)
    src_live = bool(rows_live) and len(rows_live) >= pre + post
    if not src_live:
        rows_live = db_latest(sym, tf, pre + post + 20)
    if not src_live:
        L.append("⚠ 即時取價失敗，以下為最後已收線資料")
    # 找進場那根的索引：先精確比 ts，再 fallback 比 HH:MM
    entry_idx = None
    ee_ts_ms = int(ee * 1000)
    tf_s = HA_TF.get(tf, (300,))[0]
    for i, row in enumerate(rows_live):
        rts = int(row.get("ts") or 0)
        if rts <= ee_ts_ms < rts + tf_s * 1000:
            entry_idx = i
            break
    if entry_idx is None:
        for i, row in enumerate(rows_live):
            ...
            if dt_hm == ein_hm:
                entry_idx = i
                break
    # 取進場前 pre+post 根 + 進場後所有根
    if entry_idx is not None:
        show_rows = rows_live[max(0, entry_idx - pre - post):]
        try:
            _pre_rows = rows_live[max(0, entry_idx - pre - post):entry_idx - post]
            _post_rows = rows_live[entry_idx - post:entry_idx]
            pre_sum = sum((float(r["c"]) - float(r["o"])) / float(r["o"]) * 100
                         for r in _pre_rows if r.get("c") and r.get("o"))
            post_sum = sum((float(r["c"]) - float(r["o"])) / float(r["o"]) * 100
                          for r in _post_rows if r.get("c") and r.get("o"))
        except Exception:
            pre_sum = None; post_sum = None
    else:
        show_rows = rows_live[-(pre + post):] if rows_live else []
        pre_sum = None; post_sum = None
    # 畫燈號
    _hold_dp = max(0, -S["spec"]["tick"].as_tuple().exponent)
    for i, row in enumerate(show_rows):
        abs_i = rows_live.index(row) if row in rows_live else i
        is_entry = (entry_idx is not None and abs_i == entry_idx)
        is_post_start = (entry_idx is not None and abs_i == entry_idx - post)
        if is_post_start:
            if pre_sum is not None:
        ...
        if is_entry:
            ...
            L.append("\u2504" * 18)
        act = GRN if row.get("color") == "G" else RED
        if abs_i < (entry_idx or len(rows_live)):
            if entry_idx is not None and abs_i < entry_idx - post:
                wl = RED if dr == "L" else GRN   # pre_seg：反向色
            else:
                wl = GRN if dr == "L" else RED   # post_seg：順向色
        else:
            wl = act                              # 持倉中：用 HA 色
        dt_hm = str(row.get("dt") or "")[11:16]
        ro = row.get("o"); rc = row.get("c")
        try:
            ...
            rlt = ""
        ros = f"{float(ro):.{_hold_dp}f}" if ro is not None else "-"
        lv_mark = "*" if row.get("live") else ""
        pin = " 📌" if is_entry else ""
        bad = "\u274c" if abs_i < (entry_idx or len(rows_live)) and row.get("color") != ("G" if dr == "L" else "R") and abs_i >= (entry_idx - post if entry_idx else 0) else ""
        L.append(f"{wl}{dt_hm}{act} {ros}{rlt}{bad}{lv_mark}{pin}")
    if src_live and rows_live and rows_live[-1].get("live"):
        L.append("* 為進行中，收線前仍會變動")
    # 未實現損益
    cur = None
    if mark_px:
    ...
    if cur is None:
        rr2 = db_latest(sym, tf, 1)
        if rr2: cur = Decimal(str(rr2[-1]["c"]))
...
async def cmd_run(u, c):
    ...
    # 與 /status 一致：即時抓（含進行中那根），失敗才退回 DB 已收線資料
    rows = await live_rows(sym, tf, pre + post)
    src_live = bool(rows) and len(rows) >= pre + post
    if not src_live:
        rows = db_latest(sym, tf, pre + post)
        if len(rows) < pre + post:
            await reply(u, f"{E.BOT} {E.LOSS} 資料不足（{sym} {tf} 目前 {len(rows)} 根，需 {pre+post} 根）"); return
    r = judge_entry_db(rows, dr, pre, post)
    _dp = max(0, -spec["tick"].as_tuple().exponent)
    prev = "\n".join(entry_lines_confirm(r, dr, _dp))
    if not src_live:
        prev = "⚠ 即時取價失敗，以下為最後已收線資料\n" + prev
    elif src_live and rows[-1].get("live"):
        prev += "\n* 為進行中，收線前仍會變動"
...
def build_ha_xlsx(sym, tf, ha, atrs, tick, path, kl=None):
    """明細＋統計兩張表。
    欄位：幣種 / 週期 / 日期 / 時間 / 均K燈 / 原K收 / 原K燈 / 原K漲跌幅 /
          ATR14 / ATR14 ratio"""
    ...
    pfmt = "0." + "0" * ndp
    base_font = Font(name="PingFang TC")
    for i, x in enumerate(ha):
        ...
        raw_up = (rc >= ro) if (ro is not None and rc is not None) else None
        ws.append([sym, tf, dt.strftime("%Y/%m/%d"), dt.strftime("%H:%M"),
                   "\U0001F7E9" if ha_up else "\U0001F7E5",
                   rc,
                   "\U0001F7E9" if raw_up else "\U0001F7E5" if raw_up is not None else None,
                   rbody,
        ...
        ws.cell(row=r, column=6).number_format = pfmt
        ws.cell(row=r, column=8).number_format = '0.0000"%"'
        ws.cell(row=r, column=9).number_format = pfmt
        ws.cell(row=r, column=10).number_format = '0.0000"%"'
    ws.freeze_panes = "A2"
    for row in ws.iter_rows(min_row=1, max_row=ws.max_row, max_col=len(heads)):
        for cell in row:
            if cell.row == 1:
                cell.font = Font(name="PingFang TC", bold=True, color="FFFFFF")
            else:
                cell.font = base_font
    for i, w in enumerate([12, 8, 12, 8, 8, 14, 8, 12, 14, 14], start=1):
...
async def cmd_log(u, c):
    ...
    if not ERROR_LOG:
        await reply(u, f"{E.BOT} OKX均K｜{ACCT}\n目前無錯誤記錄\n時間：{hhmmss()}"); return
    L = [f"{E.BOT} OKX均K｜{ACCT}", "事件：錯誤記錄", "━" * 10]
    for ts, label, msg in ERROR_LOG[-20:]:
        L.append(f"{ts} {label}")
        L.append(f"　{msg}")
    L += ["━" * 10, f"共 {len(ERROR_LOG)} 筆｜時間：{hhmmss()}"]
    await reply(u, "\n".join(L))
...
async def _post_init(app):
    ...
    CMDS = [BotCommand("status", "策略現況"), BotCommand("summary", "當日戰報"),
            ...
            BotCommand("replay", "逐筆回溯報表寄信"), BotCommand("log", "錯誤記錄"), BotCommand("timeframe", "週期"),
            BotCommand("menu", "說明")]
...
def main():
    ...
    for cmd, fn in [(["menu", "start"], cmd_menu), ("run", cmd_run), ("confirm", cmd_confirm),
                    ...
                    ("summary", cmd_summary), ("ha", cmd_ha), ("replay", cmd_replay), ("log", cmd_log),
