...
async def loop(app,chat,S):
    ...
    try:
        while S["alive"]:
            ...
            if reason=="__gone__":
                await notify(app,chat,f"{E.BOT} {S['sym']} {E.dir_word(d)} OKX 已無持倉（可能已手動平倉），本輪結束")
                continue
            ...
            if xr.get("code")!="0":
                ...
                await notify(app,chat,f"{E.BOT} {E.LOSS} {S['sym']} {E.dir_word(d)} 平倉失敗：{em}\n⚠ 倉位可能仍在，請至 OKX 手動處理")
                S["alive"]=False; break
            ...
            hs=int(time.time()-ee)
            log_trade({"date":today8(),"sym":S["sym"],"dir":d,"reason":reason,"ambush_s":round(ee-pt),"hold_s":hs,"gross":str(g),"fee":str(fee),"net":str(net),"nv":str(nv),"src":src,"ts":hhmmss(),"in_ts":time.strftime("%H:%M:%S",time.localtime(ee)),"tf":S["tf"],"te":str(S["te"])+"s","in_px":str(fpx),"out_px":str(xpx)})
            await notify(app,chat,f"{E.BOT} OKXLive普K｜{ACCT}\n事件：{'🟢' if net>=0 else '🔴'} 已出場\n"
                f"商　　品：{E.dir_emoji(d)} {S['sym']} {E.dir_word(d)}\n出場原因：{reason}\n進場價：{fpx}\n出場價：{xpx}\n"
                f"持倉秒數：{int(time.time()-ee)}s\n毛損益：{g:+.6f} ({gp:+.3f}%)\n手續費：{fee:+.6f} ({fp:+.3f}%)\n淨損益：{net:+.6f} ({np_:+.3f}%) {E.pnl_emoji(net)}\n時間：{hhmmss()}")
...
async def cmd_stopall(u,c):
    ...
    for k in list(alive):
        S=STRATS[k]
        if okx_pos(S["spec"]["iid"],"long" if S["dir"]=="L" else "short"): S["alive"]=False; held.append(f"{S['sym']} {S['dir']}")
        else:
            sweep_orphans(S["spec"]["iid"],"long" if S["dir"]=="L" else "short")
